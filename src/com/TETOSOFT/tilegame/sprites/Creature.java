...
public abstract class Creature extends Sprite {
    ...
    public Object clone() {
        try {
            Constructor<?> ctor = getClass().getConstructor(
                    Animation.class, Animation.class, Animation.class, Animation.class);
            return ctor.newInstance(
                    left.clone(), right.clone(), deadLeft.clone(), deadRight.clone());
        } catch (Exception ex) {
            throw new RuntimeException("Failed to clone creature: " + getClass().getName(), ex);
        }
    }
